...
int Operations::skp_x(OP_PARAMS) {
  ...
  if (proc.keyboard->isPressed(reg1)) {
    pc += 2;
  }
  return 0;
}
...
int Operations::ld_F_x(OP_PARAMS) {
  ...
  const size_t spriteAddress = reg1 * 5;
...
int Operations::ld_BCD_x(OP_PARAMS) {
  ...
  for (uint8_t i = 0, divider = 10; i < 3; i++, divider *= 10) {
    (*proc.memory)[I + i] = num % divider / (divider / 10);
  }
  return 0;
}
...
int Operations::drw_x_y_nibble(OP_PARAMS) {
  ...
  for (unsigned int i = 0; i < ctx.nibble; i++) {
    uint8_t byte      = (*proc.memory)[I + i];
    bool    wasErased = proc.display->draw(reg1, reg2, i, byte);
    if (wasErased) {
      regF = 1;
    }
  }
